/* empty css              */const w=function(){const a=document.createElement("link").relList;if(a&&a.supports&&a.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))r(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const d of i.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&r(d)}).observe(document,{childList:!0,subtree:!0});function e(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerpolicy&&(i.referrerPolicy=o.referrerpolicy),o.crossorigin==="use-credentials"?i.credentials="include":o.crossorigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(o){if(o.ep)return;o.ep=!0;const i=e(o);fetch(o.href,i)}};w();const s={"Sharkur-1.19":{title:"Sharkur",api_endpoint:"sharkur",api_version:"1.19",github:"SharkurMC/Sharkur",desc:"Active development for Minecraft 1.19.",limit:10,cache:null}};function y(t,a){return window.fetch(`https://crodylus.cezarsalat.tk/api/v2/projects/${t}/version_group/${a}/builds`).then(e=>e.status>=400?null:e.json())}function f(t){return t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;")}document.addEventListener("DOMContentLoaded",()=>{let t="",a="";Object.keys(s).length;for(const e in s){const r=s[e].title;t+=`<li class="tab"><a href="#${e}">${r}</a></li>`,a+=`
      ...
      ${a}`;for(const e in s)y(s[e].api_endpoint,s[e].api_version).then(r=>{s[e].cache=r,$(e)}).catch(r=>{console.error(r),document.getElementById(e).innerText="Failed to load downloads."});M.Tabs.init(document.querySelector("#downloads-tabs"),{onShow:e=>{history.pushState(null,null,"#"+e.getAttribute("id"))}})});function $(t){const a=s[t].github,e=document.getElementById(t).querySelector(".download-content"),r=s[t].cache;if(r==null){e.innerText="Failed to load downloads.";return}let o="",i="";const d=r.builds.filter(n=>n.downloads&&n.downloads.application);let c,h=!1;d.sort((n,l)=>l.build-n.build).slice(0,s[t].limit).forEach(n=>{let l="";n.changes.forEach(u=>{l+=`<span class="commit-hash">
                        ...
                        <td colspan="3">${b(s[t].api_endpoint)} ${n.version}</td>
                     </tr>`);const p=n.channel==="experimental";h=h||p,n.promoted;const v=p?"error":"",g=`<tr>
                        <td>
...
Channel: ${b(n.channel)}">
                                #${n.build}<i class="material-icons left">${v}</i>
                            </a>
                        </td>
            ...
            `,r.builds.length>s[t].limit&&(e.innerHTML+=`<a class="wide-btn btn light-blue darken-2 waves-effect waves-light white-text" onclick="loadMore('${t}')">More</a><br>`),s[t].api_endpoint==="paper"&&(e.innerHTML+='<a class="wide-btn btn grey darken-2 waves-effect waves-light" href="legacy">Legacy</a>'),h&&(e.innerHTML+=m)}function b(t){return t.charAt(0).toUpperCase()+t.slice(1)}
